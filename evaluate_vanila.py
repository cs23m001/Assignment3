...
def save_pred(model, dataloader):
    ...
    with open(f"./predictions_vanila/test_prediction.txt", 'w') as output:
...
test_loader, input_size, output_size, max_seq_len = get_dataloader(lang = 'hi', type = 'test', batch_size = 64)

# Initializing the model
model = seq2seq.Model(input_size = input_size, output_size = output_size, embedding_size = 128, hidden_size = 512,
                   num_encoder_layers = 1, num_decoder_layers = 4, cell_type = 'lstm',
                   dropout = 0.4, bidirectional = 'yes', max_seq_len = max_seq_len)
model = model.to(device)  

# Loading the pretrained model
model.load_state_dict(torch.load('./trained_model/vanila/vanila_best_model.pth'))
